...
set static "top"
set pm1 "count"
set init_pm1 inst_${pm1}
set pm1_v1 ${pm1}_up
set pm1_v2 ${pm1}_down

set pm2 "shift"
set inst_pm2 inst_${pm2}
...
set_property SCOPED_TO_CELLS {${inst_pm2}} [get_files $synthDir/${pm2_v1}/${pm2}_synth.dcp]
add_file $synthDir/${pm1_v1}/${pm1}_synth.dcp
set_property SCOPED_TO_CELLS {${init_pm1}} [get_files $synthDir/${pm1_v1}/${pm1}_synth.dcp]
link_design -mode default -reconfig_partitions {${inst_pm2} ${init_pm1}} -part ${device} -top ${static}
...
#锁定${init_pm1}的count_up布线
lock_design -level logical -cell $init_pm1
write_checkpoint -force -cell $init_pm1 $implDir/${init_config}/${init_pm1}_${pm1_v1}_route_design.dcp
file copy -force $implDir/${init_config}/${init_pm1}_${pm1_v1}_route_design.dcp $dcpDir
# 将布线结果中的inst_count替换为blackbox
update_design -cell ${init_pm1} -black_box
...
set_property SCOPED_TO_CELLS {${inst_pm2}} [get_files $synthDir/${pm2_v2}/${pm2}_synth.dcp]
add_file $synthDir/${pm1_v2}/${pm1}_synth.dcp
set_property SCOPED_TO_CELLS {${init_pm1}} [get_files $synthDir/${pm1_v2}/${pm1}_synth.dcp]
link_design -mode default -reconfig_partitions {${inst_pm2} ${init_pm1}} -part ${device} -top top
...
lock_design -level logical -cell ${init_pm1}
write_checkpoint -force -cell ${init_pm1} $implDir/${alter_config}/${init_pm1}_${pm1_v2}_route_design.dcp
file copy -force $implDir/${alter_config}/${init_pm1}_${pm1_v2}_route_design.dcp $dcpDir
...
write_bitstream -force  -cell ${init_pm1} $bitDir/pblock_${init_pm1}_${pm1_v2}_partial
close_project 
...
write_bitstream -force  -cell ${init_pm1} $bitDir/pblock_${init_pm1}_${pm1_v1}_partial
close_project 
